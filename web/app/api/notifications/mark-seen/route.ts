...
// web/app/api/notifications/mark-seen/route.ts — Mark notifications as seen
...
export async function POST(req: Request) {
  return withMutatingAuth(req, async ({ session }) => {
    ...
    try {
      ...
      await db
        .prepare(`UPDATE notifications SET seen_at = ? WHERE user_id = ? AND id IN (${placeholders})`)
        .bind(now, session.user_id, ...ids)
        .run();

      return Response.json({ ok: true, updated: ids.length });
    } catch (err) {
      return d1ErrorResponse("POST /api/notifications/mark-seen", err);
    }
  });
}
